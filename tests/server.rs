...
fn inscription_appears_on_reveal_transaction_page() {
  ...
  TestServer::spawn_with_args(&rpc_server, &[]).assert_response_regex(
    format!("/tx/{reveal}"),
    format!(".*<h1>Transaction .*</h1>.*<a href=/craftscription/{reveal}.*"),
  );
}
...
fn inscription_appears_on_output_page() {
  ...
  TestServer::spawn_with_args(&rpc_server, &[]).assert_response_regex(
    ...
    format!(".*<h1>Output <span class=monospace>{reveal}:0</span></h1>.*<a href=/craftscription/{inscription}.*"),
  );
}
...
fn inscription_page_after_send() {
  ...
  ord_server.assert_response_regex(
    format!("/craftscription/{inscription}"),
    format!(
      r".*<h1>Craftscription 0</h1>.*<dt>location</dt>\s*<dd class=monospace>{reveal}:0:0</dd>.*",
    ),
  );
  ...
  ord_server.assert_response_regex(
    format!("/craftscription/{inscription}"),
    format!(
      r".*<h1>Craftscription 0</h1>.*<dt>address</dt>\s*<dd class=monospace>bc1qcqgs2pps4u4yedfyl5pysdjjncs8et5utseepv</dd>.*<dt>location</dt>\s*<dd class=monospace>{send}:0:0</dd>.*",
    ),
  )
}
...
<div class=thumbnails>
  <a href=/craftscription/{inscription}><iframe .*></a>
</div>.*",
    ),
  );
}
...
fn inscriptions_page() {
  ...
  TestServer::spawn_with_args(&rpc_server, &[]).assert_response_regex(
    "/craftscriptions",
    format!(
      ".*<h1>Craftscription</h1>
<div class=thumbnails>
  <a href=/craftscription/{inscription}>.*</a>
</div>
.*",
    ),
  );
...
fn inscriptions_page_is_sorted() {
  ...
  for _ in 0..8 {
    let Inscribe { inscription, .. } = inscribe(&rpc_server);
    inscriptions.insert_str(0, &format!(".*<a href=/craftscription/{inscription}>.*"));
  }
...
fn inscriptions_page_has_next_and_previous() {
  ...
  TestServer::spawn_with_args(&rpc_server, &[]).assert_response_regex(
    format!("/craftscription/{b}"),
    format!(
      ".*<h1>Craftscription 1</h1>.*
<div class=inscription>
<a class=prev href=/craftscription/{a}>❮</a>
<iframe .* src=/preview/{b}></iframe>
<a class=next href=/craftscription/{c}>❯</a>
</div>.*",
    ),
  );
}
