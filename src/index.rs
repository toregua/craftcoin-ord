use {
  self::{
    cunes::{Cune, CuneId},
    entry::{
      BlockHashValue, CuneEntryValue, CuneIdValue, Entry, InscriptionEntry, InscriptionEntryValue,
...
pub(crate) use self::entry::CuneEntry;
...
define_table! { INSCRIPTION_ID_TO_CUNE, &InscriptionIdValue, u128 }
define_table! { INSCRIPTION_ID_TO_SATPOINT, &InscriptionIdValue, &SatPointValue }
define_table! { INSCRIPTION_NUMBER_TO_INSCRIPTION_ID, u64, &InscriptionIdValue }
define_table! { OUTPOINT_TO_CUNE_BALANCES, &OutPointValue, &[u8] }
...
define_table! { CUNE_ID_TO_CUNE_ENTRY, CuneIdValue, CuneEntryValue }
define_table! { CUNE_TO_CUNE_ID, u128, CuneIdValue }
...
define_table! { TRANSACTION_ID_TO_CUNE, &TxidValue, u128 }
define_table! { TRANSACTION_ID_TO_TRANSACTION, &TxidValue, &[u8] }
define_table! { WRITE_TRANSACTION_STARTING_BLOCK_COUNT_TO_TIMESTAMP, u32, u128 }
define_table! { CRC20_BALANCES, &str, &[u8] }
define_table! { CRC20_TOKEN, &str, &[u8] }
define_table! { CRC20_INSCRIBE_TRANSFER, &InscriptionIdValue, &[u8] }
define_table! { CRC20_TRANSFERABLELOG, &str, &[u8] }
define_multimap_table! { CRC20_TOKEN_HOLDER, &str, &str}
...
impl Index {
  pub(crate) fn open(options: &Options) -> Result<Self> {
    ...
    let auth: Auth = if !cookie_file.exists() {
      ...
      log::info!(
        "Connecting to Craftcoin Core RPC server at {rpc_url} using credentials from the url"
      );

      Auth::UserPass(username, password)
    } else {
      log::info!(
        "Connecting to Craftcoin Core RPC server at {rpc_url} using credentials from `{}`",
        cookie_file.display()
      );
    ...
    let database = match unsafe { Database::builder().open(&path) } {
      Ok(database) => {
        {
          ...
          index_cunes = statistics
            .get(&Statistic::IndexCunes.key())?
            .unwrap()
            .value()
            != 0;
          ...
          index_transactions = statistics
            ...
            .value()
            != 0;
          index_crc20 = statistics
            .get(&Statistic::IndexCrc20.key())?
            .unwrap()
            .value()
            != 0;
        }
        ...
        if error.kind() == io::ErrorKind::NotFound =>
      {
        ...
        tx.open_table(INSCRIPTION_ID_TO_CUNE)?;
...

        {
          ...
          index_crc20 = options.index_cunes();
          index_cunes = options.index_cunes();
          index_sats = options.index_sats;
          index_transactions = options.index_transactions;

          statistics.insert(&Statistic::IndexCrc20.key(), &u64::from(index_crc20))?;

          statistics.insert(&Statistic::IndexCunes.key(), &u64::from(index_cunes))?;
    ...
    Ok(Self {
      ...
      first_cune_height: options.first_cune_height(),
  ...
  pub(crate) fn get_unspent_outputs(&self, _wallet: Wallet) -> Result<BTreeMap<OutPoint, Amount>> {
    ...
    for outpoint in utxos.keys() {
      if outpoint_to_value.get(&outpoint.store())?.is_none() {
        return Err(anyhow!(
          "output in Craftcoin Core wallet but not in ord index: {outpoint}"
        ));
      }
    }
  ...
  pub(crate) fn has_cune_index(&self) -> bool {
    self.index_cunes
  }
  ...
  pub(crate) fn get_cune_by_id(&self, id: CuneId) -> Result<Option<Cune>> {
    Ok(
      self
        .database
        .begin_read()?
        .open_table(CUNE_ID_TO_CUNE_ENTRY)?
        .get(&id.store())?
        .map(|entry| CuneEntry::load(entry.value()).cune),
    )
  }

  pub(crate) fn cune(&self, cune: Cune) -> Result<Option<(CuneId, CuneEntry)>> {
    let rtx = self.database.begin_read()?;

    let entry = match rtx.open_table(CUNE_TO_CUNE_ID)?.get(cune.0)? {
      Some(id) => rtx
        .open_table(CUNE_ID_TO_CUNE_ENTRY)?
        .get(id.value())?
        .map(|entry| (CuneId::load(id.value()), CuneEntry::load(entry.value()))),
      None => None,
    };
  ...
  pub(crate) fn cunes(&self) -> Result<Vec<(CuneId, CuneEntry)>> {
    ...
    {
      let (id, entry) = result?;
      entries.push((CuneId::load(id.value()), CuneEntry::load(entry.value())));
    }

    Ok(entries)
  }

  pub(crate) fn get_cune_balance(&self, outpoint: OutPoint, id: CuneId) -> Result<u128> {
    if self.block_count()? >= self.first_cune_height && self.index_cunes {
      let rtx = self.database.begin_read()?;

      let outpoint_to_balances = rtx.open_table(OUTPOINT_TO_CUNE_BALANCES)?;
      ...
      while i < balances_buffer.len() {
        let (balance_id, length) = cunes::varint::decode(&balances_buffer[i..]);
        i += length;
        let (amount, length) = cunes::varint::decode(&balances_buffer[i..]);
        i += length;

        if CuneId::try_from(balance_id).unwrap() == id {
          return Ok(amount);
        }
      }
    }
  ...
  pub(crate) fn get_cune_balances_for_outpoint(
    &self,
    outpoint: OutPoint,
  ) -> Result<Vec<(SpacedCune, Pile)>> {
    if self.block_count()? >= self.first_cune_height && self.index_cunes {
      let rtx = &self.database.begin_read()?;

      let outpoint_to_balances = rtx.open_table(OUTPOINT_TO_CUNE_BALANCES)?;

      let id_to_cune_entries = rtx.open_table(CUNE_ID_TO_CUNE_ENTRY)?;
      ...
      let mut i = 0;
      while i < balances_buffer.len() {
        let (id, length) = cunes::varint::decode(&balances_buffer[i..]);
        i += length;
        let (amount, length) = cunes::varint::decode(&balances_buffer[i..]);
        i += length;
        ...
        let entry = CuneEntry::load(id_to_cune_entries.get(id.store())?.unwrap().value());
  ...
  pub(crate) fn get_cune_balance_map(
    &self,
  ) -> Result<BTreeMap<SpacedCune, BTreeMap<OutPoint, u128>>> {
    let outpoint_balances = self.get_cune_balances()?;

    let rtx = self.database.begin_read()?;

    let cune_id_to_cune_entry = rtx.open_table(CUNE_ID_TO_CUNE_ENTRY)?;

    let mut cune_balances: BTreeMap<SpacedCune, BTreeMap<OutPoint, u128>> = BTreeMap::new();

    for (outpoint, balances) in outpoint_balances {
      for (cune_id, amount) in balances {
        let spaced_cune = CuneEntry::load(
          cune_id_to_cune_entry
            .get(&cune_id.store())?
            .unwrap()
            .value(),
        )
  ...
  pub(crate) fn get_cune_balances(&self) -> Result<Vec<(OutPoint, Vec<(CuneId, u128)>)>> {
    ...
    if self.block_count()? >= self.first_cune_height && self.index_cunes {
      for entry in self
        .database
        .begin_read()?
        .open_table(OUTPOINT_TO_CUNE_BALANCES)?
        .iter()?
      {
        ...
        while i < balances_buffer.len() {
          let (id, length) = cunes::varint::decode(&balances_buffer[i..]);
          i += length;
          let (balance, length) = cunes::varint::decode(&balances_buffer[i..]);
          i += length;
          balances.push((CuneId::try_from(id)?, balance));
        }
  ...
  pub(crate) fn get_crc20_balances(&self, script_key: &ScriptKey) -> Result<Vec<Balance>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;

      let crc20_token_balance = rtx.open_table(CRC20_BALANCES)?;
  ...
  pub(crate) fn get_crc20_balance(
    &self,
  ...
  pub(crate) fn get_crc20_token_info(&self, tick: &Tick) -> Result<Option<TokenInfo>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;

      let crc20_token_info = rtx.open_table(CRC20_TOKEN)?;
      return Ok(
        crc20_token_info
  ...
  pub(crate) fn get_crc20_tokens_info(&self) -> Result<Vec<TokenInfo>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;
  ...
  pub(crate) fn get_crc20_token_holder(&self, tick: &Tick) -> Result<Vec<ScriptKey>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;

      let crc20_token_holder = rtx.open_multimap_table(CRC20_TOKEN_HOLDER)?;
  ...
  pub(crate) fn get_crc20_transferable_by_range(
    &self,
    script: &ScriptKey,
  ) -> Result<Vec<TransferableLog>, redb::Error> {
    let rtx = self.database.begin_read()?;
    let crc20_transferable_log = rtx.open_table(CRC20_TRANSFERABLELOG)?;
    let result = Ok(
  ...
  pub(crate) fn get_crc20_transferable_by_tick(
    &self,
    script: &ScriptKey,
    tick: &Tick,
  ) -> Result<Vec<TransferableLog>, redb::Error> {
  ...
  pub(crate) fn get_crc20_transferable_by_id(
    &self,
    script_key: &ScriptKey,
    ...
    if self.block_count().unwrap() >= self.first_inscription_height {
      ...
      let transferable_log_vec: Vec<TransferableLog> = crc20_transferable_log
  ...
  pub(crate) fn get_etching(&self, txid: Txid) -> Result<Option<SpacedCune>> {
    if self.block_count().unwrap() >= self.first_cune_height {
      let rtx = self.database.begin_read()?;

      let transaction_id_to_cune = rtx.open_table(TRANSACTION_ID_TO_CUNE)?;
      let Some(cune) = transaction_id_to_cune.get(&txid.store())? else {
        return Ok(None);
      };

      let cune_to_cune_id = rtx.open_table(CUNE_TO_CUNE_ID)?;
      let id = cune_to_cune_id.get(cune.value())?.unwrap();

      let cune_id_to_cune_entry = rtx.open_table(CUNE_ID_TO_CUNE_ENTRY)?;
      let entry = cune_id_to_cune_entry.get(&id.value())?.unwrap();

      Ok(Some(CuneEntry::load(entry.value()).spaced_cune()))
    } else {
      Ok(None)
    }
  }
  ...
  pub(crate) fn get_cune_by_inscription_id(
    &self,
    inscription_id: InscriptionId,
  ) -> Result<Option<SpacedCune>> {
    let rtx = self.database.begin_read()?;
    let Some(cune) = rtx
      .open_table(INSCRIPTION_ID_TO_CUNE)?
      .get(&inscription_id.store())?
      .map(|entry| Cune(entry.value()))
    else {
      return Ok(None);
    };
    let cune_to_cune_id = rtx.open_table(CUNE_TO_CUNE_ID)?;
    let id = cune_to_cune_id.get(cune.0)?.unwrap();
...
mod tests {
  ...
  #[test]
  #[ignore]
  fn missing_inputs_are_fetched_from_craftcoin_core() {
    for args in [
    ...
    for context in Context::configurations() {
      ...
      assert_regex_match!(
        context
          ...
          .unwrap_err()
          .to_string(),
        r"output in Craftcoin Core wallet but not in ord index: [[:xdigit:]]{64}:\d+"
      );
    }
  }
