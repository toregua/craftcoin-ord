...
pub(crate) struct Options {
  #[arg(long, help = "Load Craftcoin Core data dir from <CRAFTCOIN_DATA_DIR>.")]
  pub(crate) craftcoin_data_dir: Option<PathBuf>,
  #[arg(
    long = "chain",
    value_enum,
  ...
  #[arg(long, help = "Load Craftcoin Core RPC cookie file from <COOKIE_FILE>.")]
  ...
  #[arg(long, help = "Don't look for cunes below <FIRST_CUNE_HEIGHT>.")]
  pub(crate) first_cune_height: Option<u32>,
  #[arg(long, help = "Limit index to <HEIGHT_LIMIT> blocks.")]
  pub(crate) height_limit: Option<u32>,
  #[arg(long, help = "Use index at <INDEX>.")]
  pub(crate) index: Option<PathBuf>,
  #[arg(long, help = "Track crc20 tokens and balances.")]
  pub(crate) index_crc20: bool,
  #[arg(
    long,
    help = "Track location of cunes. CUNES ARE IN AN UNFINISHED PRE-ALPHA STATE AND SUBJECT TO CHANGE AT ANY TIME."
  )]
  pub(crate) index_cunes: bool,
  ...
  #[arg(long, help = "Connect to Craftcoin Core RPC at <RPC_URL>.")]
  pub(crate) rpc_url: Option<String>,
  #[arg(long, help = "Number of parallel requests to craftcoin node.")]
...
impl Options {
  ...
  pub(crate) fn first_cune_height(&self) -> u32 {
    if self.chain() == Chain::Regtest {
      self.first_cune_height.unwrap_or(0)
    } else if integration_test() {
      0
    } else {
      self
        .first_cune_height
        .unwrap_or_else(|| self.chain().first_cune_height())
    }
  }

  pub(crate) fn index_cunes(&self) -> bool {
    self.index_cunes
  }
  ...
  pub(crate) fn cookie_file(&self) -> Result<PathBuf> {
    ...
    let path = if let Some(craftcoin_data_dir) = &self.craftcoin_data_dir {
      craftcoin_data_dir.clone()
  ...
  fn format_craftcoin_core_version(version: usize) -> String {
    format!(
      "{}.{}.{}.{}",
      version / 1000000,
  ...
  pub(crate) fn craftcoin_rpc_client(&self) -> Result<Client> {
    ...
    let client =
      Client::new(&rpc_url, Auth::CookieFile(cookie_file.clone())).with_context(|| {
        format!(
          "failed to connect to Craftcoin Core RPC at {rpc_url} using cookie file {}",
          cookie_file.display()
        )
      })?;
    ...
    let rpc_chain = match client.get_blockchain_info()?.chain.as_str() {
      ...
      "signet" => Chain::Signet,
      other => bail!("Craftcoin RPC server on unknown chain: {other}"),
    };

    let ord_chain = self.chain();

    if rpc_chain != ord_chain {
      bail!("Craftcoin RPC server is on {rpc_chain} but ord is on {ord_chain}");
    }

    Ok(client)
  }

  pub(crate) fn craftcoin_rpc_client_for_wallet_command(&self, create: bool) -> Result<Client> {
    let client = self.craftcoin_rpc_client()?;

    const MIN_VERSION: usize = 1140600;

    let craftcoin_version = client.version()?;
    if craftcoin_version < MIN_VERSION {
      bail!(
        "Craftcoin Core {} or newer required, current version is {}",
        Self::format_craftcoin_core_version(MIN_VERSION),
        Self::format_craftcoin_core_version(craftcoin_version),
      );
    }
...
mod tests {
  ...
  fn othernet_cookie_file_path() {
    ...
    assert!(cookie_file.ends_with(if cfg!(target_os = "linux") {
      "/.craftcoin/signet/.cookie"
    } else if cfg!(windows) {
      r"\Craftcoin\signet\.cookie"
    } else {
      "/Craftcoin/signet/.cookie"
    }));
  }
  ...
  #[test]
  fn cookie_file_defaults_to_craftcoin_data_dir() {
    let arguments =
      Arguments::try_parse_from(["ord", "--craftcoin-data-dir=foo", "--chain=signet", "index"])
        .unwrap();
  ...
  fn rpc_server_chain_must_match() {
    ...
    assert_eq!(
      options.craftcoin_rpc_client().unwrap_err().to_string(),
      "Craftcoin RPC server is on testnet but ord is on mainnet"
    );
  }
  ...
  #[test]
  fn index_cunes_only_returns_true_if_index_cunes_flag_is_passed_and_not_on_mainnnet() {
    ...
    .index_cunes(),);
    assert!(
      !Arguments::try_parse_from(["ord", "--index-cunes", "index", "update"])
        .unwrap()
        .options
        .index_cunes(),
    );
