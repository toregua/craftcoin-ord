...
impl Cune {
  ...
  pub(crate) fn minimum_at_height(chain: Chain, height: Height) -> Self {
    ...
    let start = chain.first_cune_height();

    let end = start + SUBSIDY_HALVING_INTERVAL_10X;

    if offset < start {
      return Cune(Self::STEPS[12]);
    }
    ...
    Cune(start - ((start - end) * remainder / u128::from(INTERVAL)))
  }
  ...
  pub(crate) fn reserved(n: u128) -> Self {
    Cune(RESERVED.checked_add(n).unwrap())
  }
}

impl Serialize for Cune {
...
impl<'de> Deserialize<'de> for Cune {
  ...
  fn from_str(s: &str) -> crate::Result<Self> {
    ...
    for (i, c) in s.chars().enumerate() {
      ...
      match c {
        ...
        _ => bail!("invalid character in cune name: {c}"),
      }
    }
    Ok(Cune(x))
  }
...
mod tests {
  ...
  fn round_trip() {
    fn case(n: u128, s: &str) {
      assert_eq!(Cune(n).to_string(), s);
      assert_eq!(s.parse::<Cune>().unwrap(), Cune(n));
    }
  ...
  fn from_str_out_of_range() {
    "BCGDENLQRQWDSLRUGSNLBTMFIJAW".parse::<Cune>().unwrap_err();
  }
  ...
  fn mainnet_minimum_at_height() {
    ...
    fn case(height: u32, minimum: &str) {
      assert_eq!(
        Cune::minimum_at_height(Chain::Mainnet, Height(height)).to_string(),
        minimum,
      );
    }
  ...
  fn minimum_at_height() {
    ...
    fn case(chain: Chain, height: u32, minimum: &str) {
      assert_eq!(
        Cune::minimum_at_height(chain, Height(height as u64)).to_string(),
        minimum,
      );
    }
  ...
  fn serde() {
    let cune = Cune(0);
    let json = "\"A\"";
    assert_eq!(serde_json::to_string(&cune).unwrap(), json);
    assert_eq!(serde_json::from_str::<Cune>(json).unwrap(), cune);
  }
  ...
  fn reserved() {
    assert_eq!(
      RESERVED,
      "AAAAAAAAAAAAAAAAAAAAAAAAAAA".parse::<Cune>().unwrap().0,
    );

    assert_eq!(Cune::reserved(0), Cune(RESERVED));
    assert_eq!(Cune::reserved(1), Cune(RESERVED + 1));
  }

  #[test]
  fn is_reserved() {
    #[track_caller]
    fn case(cune: &str, reserved: bool) {
      assert_eq!(cune.parse::<Cune>().unwrap().is_reserved(), reserved);
    }
  ...
  fn steps() {
    for i in 0.. {
      match "A".repeat(i + 1).parse::<Cune>() {
        Ok(cune) => assert_eq!(Cune(Cune::STEPS[i]), cune),
        Err(_) => {
          assert_eq!(Cune::STEPS.len(), i);
          break;
        }
      }
    }
