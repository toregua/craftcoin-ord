...
impl Cunestone {
  ...
  fn decipher(transaction: &Transaction) -> Result<Option<Self>, script::Error> {
    let Some(payload) = Cunestone::payload(transaction)? else {
      return Ok(None);
    };

    let integers = Cunestone::integers(&payload);
    ...
    let cune = Tag::Cune.take(&mut fields).map(Cune);
  ...
  pub(crate) fn encipher(&self) -> Script {
    ...
    if let Some(etching) = self.etching {
      ...
      if let Some(cune) = etching.cune {
        Tag::Cune.encode(cune.0, &mut payload);
      }
...
mod tests {
  ...
  fn from_transaction_returns_none_if_decipher_returns_error() {
    assert_eq!(
      Cunestone::from_transaction(&Transaction {
        input: Vec::new(),
        output: vec![TxOut {
  ...
  fn deciphering_transaction_with_no_outputs_returns_none() {
    assert_eq!(
      Cunestone::decipher(&Transaction {
        input: Vec::new(),
        output: Vec::new(),
        lock_time: PackedLockTime::ZERO,
        version: 0,
  ...
  #[test]
  fn deciphering_valid_cunestone_with_invalid_script_returns_script_error() {
    let result = Cunestone::decipher(&Transaction {
      input: Vec::new(),
      output: vec![TxOut {
        script_pubkey: Script::from_bytes(vec![opcodes::all::OP_PUSHBYTES_4.to_u8()]),
        value: 0,
  ...
  #[test]
  fn deciphering_valid_cunestone_with_invalid_script_postfix_returns_script_error() {
  ...
  #[test]
  fn deciphering_cunestone_with_invalid_varint_returns_varint_error() {
    let result = Cunestone::decipher(&Transaction {
      input: Vec::new(),
      output: vec![TxOut {
        script_pubkey: script::Builder::new()
  ...
  #[test]
  fn non_push_opcodes_in_cunestone_are_ignored() {
    assert_eq!(
      Cunestone::decipher(&Transaction {
        input: Vec::new(),
        output: vec![TxOut {
  ...
  #[test]
  fn deciphering_empty_cunestone_is_successful() {
    assert_eq!(
      Cunestone::decipher(&Transaction {
        input: Vec::new(),
        output: vec![TxOut {
  ...
  #[test]
  fn error_in_input_aborts_search_for_cunestone() {
  ...
  #[test]
  fn deciphering_non_empty_cunestone_is_successful() {
    let payload = payload(&[0, 1, 2, 3]);
  ...
  #[test]
  fn cunestone_may_contain_multiple_edicts() {
  ...
  #[test]
  fn cunestone_may_be_in_second_output() {
    let payload = payload(&[0, 1, 2, 3]);
  ...
  #[test]
  fn cunestone_may_be_after_non_matching_op_return() {
    let payload = payload(&[0, 1, 2, 3]);
