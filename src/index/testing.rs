...
impl Context {
  ...
  #[track_caller]
  pub(crate) fn assert_cunes(
    &self,
    mut cunes: impl AsMut<[(CuneId, CuneEntry)]>,
    mut balances: impl AsMut<[(OutPoint, Vec<(CuneId, u128)>)]>,
  ) {
    let cunes = cunes.as_mut();
    cunes.sort_by_key(|(id, _)| *id);
    ...
    assert_eq!(cunes, self.index.cunes().unwrap());

    assert_eq!(balances, self.index.get_cune_balances());

    let mut outstanding: HashMap<CuneId, u128> = HashMap::new();
